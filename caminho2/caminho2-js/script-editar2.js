...
// exibe a img da tatuagem
if (urlImagemTatuagem) {
    const imgResultadoTatuagem = document.getElementById('resultado-tatuagem-nova');
    imgResultadoTatuagem.src = urlImagemTatuagem;
}


// executada quando todos os elementos carregarem completamente
window.onload = function() {
    const baseImage = document.getElementById('resultado-corpo');
    const overlayImage = document.getElementById('resultado-tatuagem-nova');
    let isDragging = false;
    ...
    // verifica se as img estão carregadas antes de habilitar o download
    baseImage.onload = overlayImage.onload = function() {
      const botaoDownload = document.getElementById('botao-baixar');
      botaoDownload.style.display = 'block';
    };
    
    // evento para arrastar a img com o mouse
    overlayImage.addEventListener('mousedown', function(event) {
      isDragging = true;
      offsetX = event.clientX - overlayImage.offsetLeft;
      offsetY = event.clientY - overlayImage.offsetTop;
    ...
    // faz o mouse mover a img da tatuagem verificando o click e armazena a posição final da img
    document.addEventListener('mousemove', function(event) {
      if (isDragging) {
    ...
    // faz a imagem permanecer onde foi colocada verificando o click
    document.addEventListener('mouseup', function() {
      isDragging = false;
    });

    // tira o comportamento do navegador de abrir a galeria ao clicar na imagem
    overlayImage.addEventListener('click', function(event) {
    event.preventDefault();
    });
};

// aumenta o tamanho da img
function aumentarTamanhoImagem() {
  const overlayImage = document.getElementById('resultado-tatuagem-nova');
  const currentWidth = overlayImage.offsetWidth;
  const currentHeight = overlayImage.offsetHeight;
...
function criarMontagem(){
  ...
  html2canvas(divParaSalvar).then(function(canvas) {
      ...
      // redireciona para a página resultado-final
      window.location.href = 'resultado-final2.html';
  });    
}
*/